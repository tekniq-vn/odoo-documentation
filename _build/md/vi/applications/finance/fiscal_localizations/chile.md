...
| ------------------------------------- | --------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------- |
| Chile - Kế toán                       | `l10n_cl`             | <p>Adds the minimal accounting features required for a company to operate in Chile under the<br>regulations and guidelines.</p>                    |
| Chile - Accounting Reports            | `l10n_cl_reports`     | Adds the _Propuesta F29_ and _Balance Tributario (8 columnas)_ reports.                                                                            |
| Chile - Hoá đơn điện tử               | `l10n_cl_edi`         | <p>Includes all technical and functional requirements to generate and receive electronic<br>invoices online based on the regulations.</p>          |
| Chile - Electronic Receipt            | `l10n_cl_boletas`     | <p>Includes all technical and functional requirements to generate and receive electronic<br>invoices digitally based on the regulations.</p>       |
| Electronic Exports of Goods for Chile | `l10n_cl_edi_exports` | <p>Includes technical and functional requirements to generate electronic invoices for exporting<br>goods based on the and customs regulations.</p> |
| Chile - E-Invoicing Delivery Guide    | `l10n_cl_edi_stock`   | <p>Includes all technical and functional requirements to generate delivery guides via web<br>service based on the regulations.</p>                 |
...
* Địa chỉ:
  ...
  * ZIP
  * Quốc gia
* Tax ID: enter the identification number for the selected [Taxpayer Type](chile.md#chile-fiscal-info).
...
* [Thông tin tài chính](chile.md#chile-fiscal-info)
* [Electronic invoice data](chile.md#chile-electronic-invoice)
* [DTE incoming email server](chile.md#chile-dte-email)
* [Chứng thư chữ ký](chile.md#chile-certificate)
...
* SII - Demo Mode: files are created and accepted automatically in demo mode but are**not** sent to the . For this reason, rejection\
  errors or _Accepted with Objections_ will not appear in this mode. Every internal validation can\
...
![Required information for electronic invoice.](../../../_images/electronic-invoice-data.png)
...
you want to use _Email Box Electronic Invoicing_ as the incoming email server.

#### IMPORTANT

In order to receive your SII documents, it's necessary to set up your own email server. More\
information on how to do this can be found in this documentation:[Communication in Odoo by email](../../general/email_communication.md)
...
![Incoming email server configuration for Chilean DTE.](../../../_images/dte-incoming-email.png)
...
![Chilean electronic invoice data for partners.](../../../_images/dte-email-electronic-invoice.png)
...
![Chilean fiscal document types list.](../../../_images/chilean-document-types.png)
...
The _purchase_ transactions can be managed with a single journal, but sometimes companies use more\
...
* Point of sale type: if the sales journal will be used for electronic documents, the\
  ...
  imported from a previous system or if you are using the portal _Facturación MiPyme_, you can use the option Manual.
...
Configuring these fields is required for one of the debit notes [use cases](chile.md#chile-use-cases).

## CAF

A _folio authorization code_ (CAF) is required for each document type that will be issued\
...
![Diagram with Electronic invoice transactions.](../../../_images/electronic-invoice-workflow.png)
...
![Customer invoice document type selection.](../../../_images/customer-invoice-document-type.png)
...
* The file is created automatically and recorded\
  in the chatter.
* The\
  status is set as Pending to be sent.![DTE XML File displayed in chatter.](../../../_images/xml-creation.png)
...
2.  Once the response is received, Odoo updates the\
    ...
    Rejected.![Identification transaction for invoice and Status update.](../../../_images/dte-status-steps.png)
    ...
    **IMPORTANT**
    ...
    SII for smooth processing.![Electronic invoice data statuses.](../../../_images/chatter-internal-statuses.png)
...
![Crossed referenced document(s).](../../../_images/cross-reference-tab-registration.png)
...
![Fiscal elements and barcode printed in accepted invoices.](../../../_images/accepted-invoice-fiscal-information.png)
...
![Message with the commercial acceptance from the customer.](../../../_images/partner-dte-status.png)
...
with a credit note to either cancel the invoice or correct it. Please refer to the[Giấy báo có](chile.md#chile-credit-notes) section for more details.

![Invoice Commercial status updated to claimed.](../../../_images/accepted-invoice.png)
...
*   **Lỗi:** `CARATULA`\
    **Hint:** there are just five reasons why this error could show up and all of them are related\
    to the _Caratula_ section of the XML:\
...
Khi cần hủy bỏ hoặc điều chỉnh một hóa đơn đã được xác nhận, bắt buộc phải tạo một giấy báo có. Lưu ý quan trọng rằng tệp là bắt buộc đối với giấy báo có, được xác định là Loại tài liệu 61 trong hệ thống . Vui lòng tham khảo mục [Hướng dẫn CAF](chile.md#chile-caf-documentation) để biết thêm thông tin về quy trình tải cho từng loại tài liệu.

![Creation of CAF for Credit notes.](../../../_images/credit-note-document-type.png)
...
![Credit note canceling the referenced document.](../../../_images/credit-note-cancel-ref-doc.png)
...
![Credit note correcting referenced document text.](../../../_images/credit-note-correct-text.png)

Odoo creates a credit note with the corrected text in an invoice and Price `0.00`.

![Credit note with the corrected value on the invoice lines.](../../../_images/text-correction-label.png)
...
![Credit note for partial refund to correct amounts, using the SII reference code 3.](../../../_images/credit-note-correct-amount.png)
...
![Debit note correcting referenced document amount.](../../../_images/debit-note-correct-amount.png)

In this case Odoo automatically includes the Source Invoice in the Cross\
Reference tab.

![Automatic reference to invoice in a debit note.](../../../_images/auto-ref-debit-note.png)
...
![Debit note to cancel the referenced document (credit note).](../../../_images/debit-note-cancel-ref-doc.png)
...
![Button for accepting vendor bills.](../../../_images/accept-vendor-bill-btn.png)
...
rejected.](../../../_images/claim-vendor-bill-btn.png)
...
Once all configurations have been made for [electronic invoices](chile.md#chile-electronic-invoice)\
(e.g., uploading a valid company certificate, setting up master data, etc.), the electronic\
purchase invoices need their own . Please refer to the[CAF documentation](chile.md#chile-caf-documentation) to check the details on how to acquire the\
...
purchase journal with the _Use Documents_ feature enabled. It is possible to modify an existing\
...
Odoo automatically updates the _DTE Status_ every night using a scheduled action. To get a response\
...
The _Delivery Guide_ module includes the ability to send the to and the stamp in PDF reports for\
deliveries.

Once all configurations have been made for [electronic invoices](chile.md#chile-electronic-invoice)\
(e.g., uploading a valid company certificate, setting up master data, etc.), delivery guides need\
their own . Please refer to the [CAF documentation](chile.md#chile-caf-documentation) to check the details on how to acquire the for electronic Delivery Guides.
...
![Create Delivery Guide button on a sales process.](../../../_images/delivery-guide-creation-btn.png)
...
![First Delivery Guide number warning message.](../../../_images/delivery-guide-number-warning.png)
...
![Chatter notes of Delivery Guide creation.](../../../_images/chatter-delivery-guide.png)
...
printed (if hosted in _Odoo SH_ or on _On-premise_ remember to manually add the\
pdf417gen library mentioned in the [Invoice PDF report section](chile.md#chile-electronic-invoice-pdf-report)).
...
need their own . Please refer to the [CAF documentation](chile.md#chile-caf-documentation) to check the details on how to acquire the for electronic receipts.
...
![Electronic Receipt module.](../../../_images/electronic-receipt-customer.png)
...
![Document type 39 for Electronic Receipts.](../../../_images/document-type-39.png)
...
![Electronic Receipts STE creation status.](../../../_images/electronic-receipt-ste-status.png)

The DTE Status is automatically updated by Odoo with a scheduled action that runs every\
day at night. To get a response from the immediately,\
press the Send now to SII button.

Please refer to the [DTE Workflow](chile.md#chile-electronic-invoice-validation) for electronic\
...
documentation](chile.md#chile-caf-documentation) to check the details on how to acquire the for electronic receipts.
...
![Taxpayer Type needed for the Electronic Exports of Goods module.](../../../_images/taxpayer-type-export-goods.png)
...
![PDF report section for the Electronic Exports of Goods PDF Report.](../../../_images/pdf-report-section.png)

### eCommerce electronic invoicing

To install the Chilean eCommerce module, go to Apps, search for the\
module by its technical name \`l10n\_cl\_edi\_website\_sale, and click the Activate button.

![phân hệ l10n\_cl Thương mại điện tử.](../../../_images/ecommerce-module-chile.png)
...
Once all of the configurations are made for the Chilean [electronic invoice](chile.md#chile-electronic-invoice) flow, the following configurations are required for the eCommerce flow\
to be integrated.
...
![Invoice Policy and Automatic Invoice configurations.](../../../_images/website-configurations-ecommerce-chile.png)

Since an online payment needs to be confirmed for the _automatic invoice_ feature to generate the\
...
![Invoice Policy configuration in Products.](../../../_images/ordered-quantities-product.png)
...
![Option for EDI Documents for clients.](../../../_images/select-edi-docs-ecommerce.png)
...
![Fiscal fields required for an Invoice to be requested.](../../../_images/fiscal-fields-invoice-ecommerce.png)

If the client selects the Electronic Receipts option, they will be directed to the next\
step, and the electronic document will be generated for the _Consumidor Final Anónimo_ contact.
...
company to generate an electronic export invoice (_document type 110_), which can be done from\
the _Accounting_ app.

### Point of Sale electronic invoicing

To install the Chilean Module for Point of Sale, go to the Apps\
application on the main Odoo dashboard, search for the module by its technical name`l10n_cl_edi_pos`, and click the Activate button.

![phân hệ l10n\_cl POS EDI.](../../../_images/pos-edi-module-chile.png)
...
To configure contacts with the required fiscal information, review the [partner information](chile.md#chile-partner-information) section, or directly modify a contact. Navigate to\
...
![Contact with fiscal information created from POS.](../../../_images/fiscal-required-pos-session.png)
...
the product as Available for POS, this makes the product available for sale in th&#x65;_&#x50;oint of Sale_ app.

![Product with fiscal information created from POS.](../../../_images/available-in-pos-product.png)
...
![Configuration to generate QR or 5 digit codes on tickets.](../../../_images/qr-code-ticket.png)
...
![Automatic contact selection of an anonymous end consumer.](../../../_images/invoice-receipt-selection.png)

#### NOTE

If the client requests a credit note due to a return of their purchase, the credit note should be\
made using the _Accounting_ app. See the [credit notes and refunds](../accounting/customer_invoices/credit_notes.md) documentation for detailed instructions.
...
![Selection of contact for the receipt.](../../../_images/contact-for-electronic-invoice.png)
...
![Selection of invoice option at payment.](../../../_images/invoice-option-at-payment.png)
...
For electronic receipts (not generated for the _Consumidor Final Anónimo_) and electronic invoices,\
it is possible to manage the process to return products sold in a order\
by selecting the Refund button.

![Refund option in the POS application.](../../../_images/refund-order.png)
...
![Selection of order for the refund process.](../../../_images/select-order-refund.png)
...
![Location of the Reporte Balance Tributario de 8 Columnas.](../../../_images/locate-fiscal-balance-report.png) ![Chilean Fiscal Balance (8 Columns).](../../../_images/8-col-fiscal-balance-report.png)

### Propuesta F29

The form _F29_ is a new system that the enabled to\
taxpayers, and that replaces the _Purchase and Sales Books_. This report is integrated by Purchase\
...
The _Propuesta F29 (CL)_ report in Odoo covers the basic legal requirements as a first proposal\
...
![Location of the Propuesta F29 (CL) Report.](../../../_images/locate-propuesta-f29-report.png)

It is possible to set the and the\
Proportional Factor for the fiscal year from the Accounting ‣\
Configuration ‣ Settings.

![Default PPM and Proportional Factor for the Propuesta F29 Report.](../../../_images/f29-report.png)

Or manually in the reports by clicking on the ✏️ (pencil) icon.

![Manual PPM for the Propuesta F29 Report.](../../../_images/manual-ppm-f29-report.png)
