...
Each payment provider is linked to a list of supported [payment methods](payment_providers.md#payment-providers-payment-methods) that can be (de)activated based on your needs.
...
|                                                                         | Payment flow from          | [Token hoá](payment_providers.md#payment-providers-tokenization) | [Thu hồi thủ công](payment_providers.md#payment-providers-manual-capture) | [Hoàn tiền](payment_providers.md#payment-providers-refunds) | [Thanh toán nhanh](payment_providers.md#payment-providers-express-checkout) |
| ----------------------------------------------------------------------- | -------------------------- | ---------------------------------------------------------------- | ------------------------------------------------------------------------- | ----------------------------------------------------------- | --------------------------------------------------------------------------- |
...
* The fields available in the Credentials tab depend on the payment provider. Refer\
  to the [related documentation](payment_providers.md#payment-providers-supported-providers) for more\
  information.
...
‣ Configuration ‣ Payment Methods. Click on the payment method, then activate the[developer mode](../general/developer_mode.md#developer-mode). Click the Configuration tab to adapt the\
features.

## Token hoá

[If the payment provider supports this feature](payment_providers.md#payment-providers-online-providers), customers\
...
[If the payment provider supports this feature](payment_providers.md#payment-providers-online-providers), you can\
...
* Some payment providers support capturing only part of the authorized amount. The remaining\
  ...
  **partial** in the [table above](payment_providers.md#payment-providers-online-providers). The providers that\
...
* Some payment providers support refunding only part of the amount. The remaining amount can then\
  optionally be refunded, too. These providers have the value **Full and partial** in the[table above](payment_providers.md#payment-providers-online-providers). The providers that only support\
...
* [Payment methods](payment_providers.md#payment-providers-payment-methods) also have their own list of\
...
Từ góc độ kế toán, có hai loại quy trình thanh toán online: các khoản thanh toán được chuyển trực tiếp vào tài khoản ngân hàng của bạn và tuân theo quy trình [đối chiếu](accounting/bank/reconciliation.md) thông thường, và các khoản thanh toán từ [nhà cung cấp dịch vụ thanh toán online bên thứ ba](payment_providers.md#payment-providers-online-providers) yêu cầu bạn tuân theo một quy trình kế toán khác. Đối với các khoản thanh toán này, bạn cần xem xét cách ghi nhận bút toán sổ nhật ký. Chúng tôi khuyên bạn nên tham khảo ý kiến kế toán viên.

By default, the Bank Account defined for the [payment journal](payment_providers.md#payment-providers-journal) is used, but you can also specify an [outstanding account](accounting/bank.md#bank-outstanding-accounts) for each payment provider to separate the provider's payments from\
other payments.

![Define an outstanding account for a payment provider.](../../_images/bank_journal.png)
