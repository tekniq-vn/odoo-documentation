...
_Reordering rules_ are used to keep forecasted stock levels above a certain threshold without\
...
Quy tắc tái đặt hàng có thể được cấu hình cho từng sản phẩm dựa trên tuyến được sử dụng để bổ sung sản phẩm đó. Nếu sản phẩm sử dụng tuyến _Mua hàng_, một _yêu cầu báo giá_ (RFQ) sẽ được tạo khi quy tắc tái đặt hàng được kích hoạt. Thay vào đó, nếu sản phẩm sử dụng tuyến _Sản xuất_, thì một _lệnh sản xuất_ (MO) sẽ được tạo ra. Điều này áp dụng bất kể tuyến bổ sung hàng nào được chọn.
...
* [Reordering rules setup](reordering_rules.md#inventory-warehouses-storage-configure-rr)
* [Trigger](reordering_rules.md#inventory-product-management-trigger)
* [Preferred route](reordering_rules.md#inventory-warehouses-storage-route)

To understand and optimize replenishment using advanced features, see:

* [Just-in-time logic](reordering_rules.md#inventory-warehouses-storage-just-in-time)
* [Visibility days](reordering_rules.md#inventory-product-management-visibility-days)

## Reordering rules setup

To configure automatic and manual reordering rules, complete the following:

1. [Product type configuration](reordering_rules.md#inventory-warehouses-storage-set-product-type)
2. [Create rule](reordering_rules.md#inventory-warehouses-storage-rr-fields)
...
![Select one or more routes on the Inventory tab.](../../../../../_images/select-routes1.png)
...
![Specify a vendor and price on the Purchase tab.](../../../../../_images/specify-vendor.png)

If the product is replenished using the Manufacture route, it needs to have at least on&#x65;_&#x62;ill of materials_ (BoM) associated with it. This is necessary because Odoo only creates\
...
![The Bill of Materials smart button on a product form.](../../../../../_images/bom-smart-button.png)
...
![The form for creating a new reordering rule.](../../../../../_images/reordering-rule-form.png)

#### NOTE

To learn how the On Hand, Forecast, and To Order fields are\
calculated using on-hand quantities and future demand, see the [Just-in-time logic](reordering_rules.md#inventory-warehouses-storage-just-in-time) section.

For advanced usage of reordering rules, learn about the following reordering rule fields:

* [Trigger](reordering_rules.md#inventory-product-management-trigger)
* [Preferred route](reordering_rules.md#inventory-warehouses-storage-route)
* [Nhà cung cấp](reordering_rules.md#inventory-warehouses-storage-set-vendor)
* [Bill of materials](reordering_rules.md#inventory-warehouses-storage-set-bom-field)
* [Procurement group](reordering_rules.md#inventory-warehouses-storage-procurement-grp)
* [Visibility days](reordering_rules.md#inventory-product-management-visibility-days)

#### NOTE

The fields above are not available by default, and must be enabled by selecting the(adjust) icon in the far-right corner and selecting the\
desired column from the drop-down menu.

### 0/0/1 reordering rule

The _0/0/1_ reordering rule is a specialty rule used to replenish a product that is not kept\
on-hand, each time a sales order (SO) is confirmed for that product.

#### IMPORTANT

The 0/0/1 reordering rule is similar to the _Replenish on Order (MTO)_ route, in that both\
workflows are used to replenish a product upon confirmation of an .

The main difference between the two methods is that the _Replenish on Order_ route automatically\
reserves the product for the that caused it to be replenished. This means the product**cannot** be used for a different .
...
Another key difference is that replenishment orders created by the _Replenish on Order_ route are\
...
At the top of the product's page, click the Reordering Rules smart\
...
![A 0/0/1 reordering rule.](../../../../../_images/001-rule.png)
...
A reordering rule's _trigger_ can be set to _automatic_ or _manual_. While both function the same\
way, the difference between the two types of reordering rules is how the rule is launched:

* [Auto](reordering_rules.md#inventory-warehouses-storage-auto-rr): A purchase or manufacturing order is\
...
* [Manual](reordering_rules.md#inventory-warehouses-storage-manual-rr): The [Replenishment report](report.md)\
  lists products needing replenishment, showing current/forecasted stock, lead times, and arrival\
  dates. Users can review forecasts before clicking _Order Once_.
...
the (adjust) icon, located to the far-right of the column\
...
_Automatic reordering rules_, enabled by setting the reordering rule's Trigger field to\
...
2. A sales order is confirmed, and lowers the _Forecasted_ quantity of the product below the\
   minimum.
...
_Manual reordering rules_, configured by setting the reordering rule's Trigger field to\
Manual, list a product on the [replenishment dashboard](report.md) when the\
forecasted quantity falls below a specified minimum. Products on this dashboard are called _needs_,\
...
![Click the Order Once button on the replenishment dashboard to replenish stock.](../../../../../_images/manual.png)
...
![Select a preferred route from the drop-down.](../../../../../_images/select-preferred-route.png)
...
*   Procurement Group: This is a way to group related or that are tied to\
    ...
    **NOTE**
    ...
    using the [MTO route](mto.md).![Showing smart button to PO.](../../../../../_images/po-smartbutton.png)
...
_Just-in-time logic_ in Odoo minimizes storage costs by placing orders precisely to meet deadlines.\
This is achieved using the [forecasted date](reordering_rules.md#inventory-warehouses-storage-forecasted-date),\
...
* While lead times and just-in-time logic provide additional control, **reordering rules work**\
  **perfectly fine without them**. Keeping delivery dates on sales orders as their _creation date_\
  ensures purchases are immediately triggered when needed

### Forecasted date and To Order quantity

To view the _forecasted date_, go to the replenishment report and click the (info) icon for the desired reordering rule. The Replenishment Information\
pop-up window displays the Forecasted Date and various lead times.

The _forecasted date_ is the total time needed to procure a product in Odoo. It is calculated by\
...
The _just-in-time_ logic ensures replenishment happens only when it's necessary for the forecasted\
...
_Visibility days_ enable the ability to determine if additional quantities should be added to the\
...
to Inventory app ‣ Operations ‣ Replenishment, or by clicking the _Reordering_\
_Rules_ smart button from the product form.

Next, enable the Visibility Days field by clicking the (adjust) icon to the far right and choosing the feature from the drop-down menu. Then,\
...
![Show forecasted date visualization.](../../../../../_images/forecasted-date.png)
...
![Visibility days visualization.](../../../../../_images/visibility-days.png)
...
![Example where the visibility days does not trigger.](../../../../../_images/counterexample.png)
