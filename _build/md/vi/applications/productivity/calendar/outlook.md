...
Synchronizing a user's _Outlook Calendar_ with Odoo is useful for keeping track of tasks and\
...
To sync the _Outlook Calendar_ with Odoo's _Calendar_, a Microsoft _Azure_ account is required.\
Creating an account is free for users who have never tried, or paid for, _Azure_. For more\
information, view the account options on the [Azure website](https://azure.microsoft.com/en-us/free/?WT.mc_id=A261C142F).

Refer to [Microsoft's documentation](https://docs.microsoft.com/en-us/azure/active-directory/develop/quickstart-create-new-tenant) on how to set up a Microsoft _Entra ID_ (formally called\
Microsoft _Azure Active Directory (Azure AD)_). This is an API console to manage and register\
Microsoft applications.

Existing Microsoft _Entra ID_ users should log in at the [Microsoft Azure developer portal](https://portal.azure.com/#home). Next, select View under the section labeled\
...
application will not work. Users who wish to connect their _Outlook Calendar_ to Odoo should select\
...
drop-down menu. Then, enter the Odoo database URI (URL) followed by`/microsoft_account/authentication`.
...
it is needed later, in the [Cấu hình trong Odoo](outlook.md#outlook-calendar-odoo-setup).
...
The second credential needed to complete the synchronization of the Microsoft _Outlook Calendar_ is\
the _Client Secret_. The user **must** add a client secret, as this allows Odoo to authenticate\
itself, requiring no interaction from the user's side. _Certificates_ are optional.
...
secret when created _before_ leaving the page.
...
From the Microsoft _Azure_ portal, under the Overview section of the application, copy\
...
In the Odoo database, open to the _Calendar_ module, and click the Outlook sync button\
...
(_Outlook_ and Odoo). The page redirects to a Microsoft login page, and the user is asked to log in\
...
Outlook](outlook.md#outlook-sync). The configuration of Microsoft's _Azure_ account is only done once, as\
Microsoft _Entra ID_ tenants' client IDs and client secrets are unique, and help the user manage\
...
There may be times when the _Microsoft Outlook Calendar_ account does not sync correctly with Odoo.\
...
The following reset options are available for troubleshooting _Microsoft Outlook Calendar_ sync with\
Odoo:
...
> * Delete from the current Microsoft Calendar account: delete the events from _Microsoft_\
>   _Outlook Calendar_.
> * Delete from Odoo: delete the events from the Odoo calendar.
> * Delete from both: delete the events from both _Microsoft Outlook Calendar_ and Odoo\
>   calendar.

Next Synchronization:

> * Synchronize only new events: sync new events on _Microsoft Outlook Calendar_ and/or\
>   Odoo calendar.
> * Synchronize all existing events: sync all events on _Microsoft Outlook Calendar_\
