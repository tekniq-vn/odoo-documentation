...
![Open the CRM app and click on the Reporting tab along the top, then click Pipeline.](applications/sales/crm/performance/win_loss/reporting-tab-and-pipeline-view.png)
...
![The default state of the Pipeline Analysis page is a graph, with many options to change it.](applications/sales/crm/performance/win_loss/pipeline-analysis-page.png)
...
- Actions: represented by the ⚙️ (gear) icon, located next to the
  ...
  Spreadsheet. (See [Save and share reports](#win-loss-save-reports) for more
  information)
...
- Search... bar: shows the filters and groupings currently being applied to the graph.
  ...
  icon, at the end of the bar, to open a drop-down menu of options. (See [Search Options](#win-loss-search) for more information)

In the upper-right corner, there are view options represented by different icons. (See [View
Options](#win-loss-view) for more information)
...
- Measures: opens a drop-down menu of different measurement options that can be seen in
  ...
  list view. (See [Measurement Options](#win-loss-measure) for more information)
...
![Clicking on the down arrow next to the search bar opens a menu of filters for the analysis.](applications/sales/crm/performance/win_loss/search-panel-filters-and-group-by-options.png)
...
- Add Custom Filter: allows the user to create a custom filter with numerous
  options. (See [Add Custom Filters and Groups](#win-loss-custom-filters) for more
  information)
...
- Add Custom Group: allows the user to create a custom group with numerous
  options. (See [Adding Custom Filters and Groups](#win-loss-custom-filters) for more
  information)
...
<a id="win-loss-custom-filters"></a>
...
![The add branch feature allows the creation of more complex all or any statements for rules.](applications/sales/crm/performance/win_loss/custom-filter-add-branch.png)
...
![The Graph View displays the analysis as a Bar Chart, Line Chart, or Pie Chart.](applications/sales/crm/performance/win_loss/graph-view.png)
...
![The Pivot View displays the analysis as a table.](applications/sales/crm/performance/win_loss/pivot-view.png)
...
![The Cohort View displays the analysis as individual weeks of the year.](applications/sales/crm/performance/win_loss/cohort-view.png)
...
![The List View displays a single list of all records matching the search criteria.](applications/sales/crm/performance/win_loss/list-view.png)
...
<a id="win-loss-reports"></a>

## Tạo báo cáo

After understanding how to [navigate the pipeline analysis page](#win-loss-pipeline), the
...
![The search criteria for win/loss reports is Created On, Stage, and Active is in true false.](applications/sales/crm/performance/win_loss/search-criteria-for-basic-win-loss.png)
...
2. On the Pipeline Analysis page, click the ⬇️ (down arrow) icon, next to
   ...
   ![The Search menu containing the filters for a basic win/loss report.](applications/sales/crm/performance/win_loss/filters-for-basic-win-loss-report.png)
...
![The Add Custom Filter menu showing two rules: (1) Active is set, and (2) Active is not set.](applications/sales/crm/performance/win_loss/add-custom-active-filter.png)
...
![A basic win/loss report showing all leads whether won or lost grouped by stage.](applications/sales/crm/performance/win_loss/basic-win-loss-report.png)

#### Customize win/loss reports

After [creating a win/loss report](#win-loss-win-loss), consider using the options below to
...
![The Search menu open and the Won and Lost filters highlighted.](applications/sales/crm/performance/win_loss/search-panel-filters-and-group-by-options.png)
...
![A win/loss report in Pivot View displays the data in table form.](applications/sales/crm/performance/win_loss/win-loss-pivot-view.png)
...
![A win/loss report in List View displays all leads in an easy-to-read list.](applications/sales/crm/performance/win_loss/win-loss-list-view.png)

<a id="win-loss-save-reports"></a>

## Save and share reports

After [creating a report](#win-loss-reports), the search criteria can be saved, so the report
...
![Under the Favorites heading, click Save current search and save the report for later.](applications/sales/crm/performance/win_loss/save-to-favorites.png)
...
> ![Pivot View reports especially benefit from being inserted in spreadsheets.](applications/sales/crm/performance/win_loss/pivot-view-in-spreadsheet.png)
...
![To access the saved report, open the Dashboard app and click My Dashboard.](applications/sales/crm/performance/win_loss/add-to-dashboard.png)
