...
to be [loaded lazily (on demand)](assets.md#frontend-assets-lazy-loading).
...
the javascript files, these files are processed (`scss` files are converted into`css`), then minified (again, if not in `debug=assets` [mode](developer/reference/frontend/framework_overview.md#frontend-framework-assets-debug-mode))\
...
Odoo assets are grouped by _bundles_. Each bundle (a _list of file paths_\
of specific types: `xml`, `js`, `css` or `scss`) is listed in the[module manifest](developer/reference/backend/module.md#reference-module-manifest). Files can be declared using[glob](https://en.wikipedia.org/wiki/Glob_\(programming\)) syntax, meaning that\
...
* `web.assets_common`: this bundle contains most assets which are common to the\
  ...
  lower level building blocks for the odoo framework. Note that it contains the`boot.js` file, which defines the odoo module system.
...
Note that all directives targeting a certain asset file (i.e. `before`, `after`,`replace` and `remove`) need that file to be declared beforehand, either\
...
Prepending a file at the beginning of a bundle might not be precise enough. The`before` directive can be used to add the given file(s) right _before_ the target\
file. It is declared by replacing the normal path with a 3-element tuple`('before', <target>, <path>)`.
...
Same as `before`, with the matching file(s) appended right _after_ the target file.\
It is declared by replacing the normal path with a 3-element tuple`('after', <target>, <path>)`.
...
can be done using the `remove` directive by specifying a pair`('remove', <target>)`:
...
lib folder. One solution would be to create an [ir.asset](assets.md#frontend-assets-ir-asset)\
