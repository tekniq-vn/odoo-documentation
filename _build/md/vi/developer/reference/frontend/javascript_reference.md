...
* the _web client_: this is the private web application, where one can view and\
...
* the _website_: this is the public part of Odoo. It allows an unidentified\
...
* the _point of sale_: this is the interface for the point of sale. It is a\
...
* using the global scope (the _window_ object) to read/write references to some\
...
sometimes necessary to modify the behavior of an existing class _in place_. The\
...
by the framework are categories on the main registry, that can be imported from`@web/core/registry`
...
For example, we have the _orm_ service whose job is to allow interacting with\
...
* _info_: useful to display some informational feedback as a consequence of an\
  action that cannot fail.
* _success_: the user performed an action that can sometimes fail but didn't.
* _warning_: the user performed an action that could only be partially completed.\
...
could be displayed with two buttons to _Accept_ or _Decline_.

### Displaying notifications

There are two ways to display notifications in Odoo:

* The _notification_ service allows component to display notifications from JS\
  code by calling the add method.
* The _display\_notification_ client action allows to trigger the display\
...
* _message_: string, optional. The content of the notification. Can be a markup\
  object to display formatted text.
* _sticky_: boolean, optional (default false). If true, the notification\
  will stay until the user dismisses it. Otherwise, the notification will\
  be automatically closed after a short delay.
* _type_: string, optional (default "warning"). Determines the style of the\
  notification. Possible values: "info", "success", "warning", "danger"
* _className_: string, optional. This is a css class name that will be\
...
* each translatable string is tagged with the special function _\_t_
* these strings are used by the server to generate the proper PO files
* whenever the web client is loaded, it will call the route _/web/webclient/translations_,\
...
retrieve the translated string _before_ using it for interpolation.
...
Each decoration _decoration-X_ will be mapped to a css class _text-X_, which is\
a standard bootstrap css class (except for _text-it_ and _text-bf_, which are\
...
*   `min_date` / `max_date`: sets limit dates for accepted values. By default, the earliest\
    ...
    > ```
*   warn\_future: displays a warning if the value is in the future (based on today).
...
* see [Date Field](javascript_reference.md#reference-javascript-reference-date-field) options
...
* see [Date Field](javascript_reference.md#reference-javascript-reference-date-field) or [Date & Time Field](javascript_reference.md#reference-javascript-reference-datetime-field) options
...
\
Options:\
\- \`website\_path\`: (default: \`false\`) by default, the widget forces (if not already\
...
\
Options:\
\- \`accepted\_file\_extensions\`: the file extension the user can pick from the file input dialog box\
\> (cf: \`accept\` attribute on \`\`)
...
hardcoded fields, present in the view: `stage_id`, `legend_normal`,`legend_blocked`, `legend_done`. This is mostly used to display and change\
...
by using the [Decoration](javascript_reference.md#reference-javascript-reference-field-decoration) mechanism.\
...
clickable link in this case. In edit, it behaves exactly like the regular`many2one`.\
...
subset of the choices. Note that the number of displayed values is limited to`100`. This limit isn't customizable. It simply allows to handle extreme cases\
...
Reference (`reference`)\
: The `reference` field is a combination of a select (for the model) and a`many2one` field (for its value). It allows the selection of a record on an\
arbitrary model.\
...
* from the perspective of the server, it is a record of the model _ir\_action_,\
  with a field _tag_ of type char
...
action record (a record of the model `ir.actions.client`) with the proper`tag` attribute:
