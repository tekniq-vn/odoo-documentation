...
QWeb is the primary [templating](https://en.wikipedia.org/wiki/Template_processor) engine used by Odoo[<sup>2</sup>](qweb.md#othertemplates). It\
is an XML templating engine[<sup>1</sup>](qweb.md#genshif) and used mostly to generate [HTML](https://en.wikipedia.org/wiki/HTML)\
fragments and pages.

Template directives are specified as XML attributes prefixed with `t-`,\
for instance `t-if` for [Conditionals](qweb.md#reference-qweb-conditionals), with elements\
...
See [Advanced Output](qweb.md#reference-qweb-advanced-output) for more advanced topics (e.g.\
...
`*$as*_last`\
: whether the current item is the last of the iteration (equivalent to`*$as*_index + 1 == *$as*_size`), requires the iteratee's size be\
available
...
These extra variables provided and all new variables created into the`foreach` are only available in the scope of the `foreach`. If the\
...
The body of the `call` directive can be arbitrarily complex (not just`set` directives), and its rendered form will be available within the called\
...
The only cross-platform "safe" content is the output of[t-call](qweb.md#reference-qweb-call) or a [t-set](qweb.md#reference-qweb-set)\
...
*   `markupsafe.Markup`.
    ...
    **WARNING**
    ...
    `markupsafe.Markup` is an unsafe API, it's an _assertion_\
...
to a normal string to "strip" the safety flag e.g. `str(content)` in Python and`String(content)` in Javascript.

**NOTE**

Because `Markup` is a much richer type than`Markup()`, some operations will strip the safety information from\
a `Markup()` but not a `Markup` e.g. string\
concatenation (`'' + content`) in Python will result in a`Markup` with the other operand having been properly\
escaped, while in Javascript will yield a `String()` where the\
other operand was _not_ escaped before the concatenation.
...
`raw`\
: A version of `out` which _never_ escapes its content. Content is emitted\
...
`t-raw` was deprecated because as the code _producting_ the content\
...
default).\
The behaviour can be configured via [`PYTHONBREAKPOINT`](https://docs.python.org/3/using/cmdline.html#envvar-PYTHONBREAKPOINT) or[`sys.breakpointhook()`](https://docs.python.org/3/library/sys.html#sys.breakpointhook).
...
remain dynamic and be evaluated at cache times. However, the part in`t-nocache` will not have access to the `t-set` value of the template. Only\
...
**expression**, then the model, ids and their corresponding write\_date will be\
...
condition. And if a module modifies the record, the write\_date being modified,\
...
Here the `<i>` tag that contains the container will always be rendered. While\
the rest is as a single string in the cache. The counter is not updated by the`t-set` out of the `t-nocache`
...
in which case templates stored in the database (as[views](developer/reference/user_interface/view_architectures.md#reference-view-architectures-qweb)) can be trivially rendered by calling`odoo.http.HttpRequest.render()`:
...
At a deeper level than the previous helper is the `_render` method on`ir.qweb` (use the datable) and the public module method `render`\
...
* **Tham số:**
  ...
  * **engine** ([_str_](https://docs.python.org/3/library/stdtypes.html#str)) -- name of the Odoo model to use for rendering, can be\
...
on the extended template to select _context nodes_ to which the specified`t-operation` is applied:
...
(core is the `web.core` module) An instance of [`QWeb2.Engine()`](qweb.md#QWeb2.Engine) with all module-defined template\
...
A [`QWeb2.Engine()`](qweb.md#QWeb2.Engine) also serves as a "template namespace".

#### QWeb2.Engine.QWeb2.Engine.render(template)

Renders a previously loaded template to a String, using`context` (if provided) to find the variables accessed\
...
Odoo Web, Kanban views get their own [`QWeb2.Engine()`](qweb.md#QWeb2.Engine)\
...
A [`QWeb2.Engine()`](qweb.md#QWeb2.Engine) also exposes various attributes for\
