...
Module data is declared via [data files](../reference/backend/data.md#reference-data), XML files with`<record>` elements. Each `<record>` element creates or updates a database\
record.
...
The gantt view requires the web\_gantt module which is present in the [enterprise edition](../../#install-editions) version.
...
Odoo uses a report engine based on [QWeb Templates](../reference/frontend/qweb.md#reference-qweb),[Twitter Bootstrap](https://getbootstrap.com) and [Wkhtmltopdf](https://wkhtmltopdf.org).
...
* **\[1]** it is possible to [disable the automatic creation of some fields](../reference/backend/orm.md#reference-fields-automatic-log-access)
