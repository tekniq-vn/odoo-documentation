...
Module data is declared via [data files](developer/reference/backend/data.md#reference-data), XML files with`<record>` elements. Each `<record>` element creates or updates a database\
record.
...
Odoo uses a report engine based on [QWeb Templates](developer/reference/frontend/qweb.md#reference-qweb),[Twitter Bootstrap](https://getbootstrap.com) and [Wkhtmltopdf](https://wkhtmltopdf.org).
